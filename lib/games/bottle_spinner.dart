...
class _MainState extends State<Main> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    Vector2 position = Vector2(startDXPoint - width/2, height/2-startDYPoint);
    angle = position.x>0 ? atan(position.y/position.x) : pi+atan(position.y/position.x);
    Vector2 tanVel = vel-position*dot2(vel, position)/dot2(position, position);
    bool direction = cross2(position, tanVel)>0;
    print(direction);
    double w0 = (direction?1 : -1)*sqrt(dot2(tanVel, tanVel))/sqrt(dot2(position, position));
        ...
        child: Column(
          ...
          children: <Widget>[
            AnimatedBuilder(
              ...
              child: Container(
                child: Image.asset('images/alcohol_bottle2.png'),
              ),
              builder: (BuildContext context, Widget _widget) {
                return new Transform.rotate(
                  angle: isDragging ? -angle : -angle+alpha*_duration*_duration*_controller.value*(1-0.5*_controller.value),
                  child: _widget,
                );
              },
            ),
  ...
  void _onDragStartHandler(DragStartDetails details) {
    ...
    setState(() {
      ...
      this.startDXPoint = double.parse(details.globalPosition.dx.toStringAsFixed(3));
      this.startDYPoint = double.parse(details.globalPosition.dy.toStringAsFixed(3));
    });
  }
